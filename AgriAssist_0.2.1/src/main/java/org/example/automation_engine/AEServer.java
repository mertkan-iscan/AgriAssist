...
public class AEServer {
    ...
    private final int commandPort = 54321;    // Port for Desktop App
    private final int sensorPort = 12345;     // Port for sensors
    private final int actuatorPort = 32154;   // Port for actuators


    private static Socket desktopAppSocket; // Single socket for the desktop app
    ...
    public static void sendDataToApp(String data) {
        if (desktopAppSocket != null && !desktopAppSocket.isClosed()) {
            try {
                PrintWriter out = new PrintWriter(desktopAppSocket.getOutputStream(), true);
                out.println(data);
                System.out.println("Data sent to Desktop App: " + data);
            } catch (IOException e) {
